% fc_animate2(mname,nstart,nend,cval)
...
% Identical to fc_animate except accepts cval as input.
...
% cval: caxis (image cutoff) value
%  Images will be truncated at +/- cval
...
function mv = fc_animate2(mname,nstart,nend,cval)
% cd C:\aardvark\nufdtd3d_mur\resJune2k6\s3
...
for n=nstart:nend  % For each desired output time step
    ...
    grid off
    caxis([-cval,cval])  % Dependent axis range
    colorbar
    axis image
